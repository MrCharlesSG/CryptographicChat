...
class User:
    def __init__(self, username, pbk, prk, enc_key, decrypted_server_enc_key=None):
        self.id = str(os.urandom(8).hex())
        self.username = username
        self.public_key = pbk
        self.private_key_to_store = prk
    ...
    def create_user(username, password, server_public_key):
        ...
        prk_encrypted = AESCipher(password).encrypt(prk)
    ...
    def decrypt_prk(self, password):
        return AESCipher(password).decrypt(self.private_key_to_store)
    ...
    def to_dict(self):
        return {
            ...
            "private_key_to_store": self.private_key_to_store,
