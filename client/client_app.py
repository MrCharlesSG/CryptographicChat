...
from server.config.dummy_db import server_public_key
from model.DecryptedChat import DecryptedChat
from client.service.AuthService import ClientAuthService
from client.service.ChatService import ClientChatService
...
def is_authenticated():
    return "private_key_to_store" in session
...
def api_login():
    ...
    if username:
        send_to_login = ClientAuthService.send_login(username, server_public_key)
...
def login_set_password():
    ...
    if "temporal_server_data" in session:
        ...
        rtn = ClientAuthService.receive_login(server_data, password, server_public_key)
        print("The decrypted things ", rtn)
        session["private_key_to_store"] = rtn["private_key_to_store"]
...
def api_register():
    ...
    if username and password:
        cred = ClientAuthService.send_register(username, password, server_public_key)
...
def logout():
    if is_authenticated():
        ...
        session.pop("private_key_to_store")
...
def register_repeat_password():
    ...
    if "temporal_server_data" in session:
        server_data = session.pop("temporal_server_data")
        rtn = ClientAuthService.receive_register(server_data, password, server_public_key)
        print("The decrypted things ", rtn)
        session["private_key_to_store"] = rtn["private_key_to_store"]
        session["server_enc_key"] = rtn["server_enc_key"]
        session["public_key"] = rtn["public_key"]
